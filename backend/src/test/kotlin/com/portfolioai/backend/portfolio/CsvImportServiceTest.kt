...
class CsvImportServiceTest {
    ...
    fun `preview skips footer row with fewer than 10 columns`() {
        val footer = "À partir de 2025-01-01"   // single-column footer line
        val file = makeCsv(row(), footer)
        val preview = service.preview(file)

        assertEquals(1, preview.totalItems)   // footer not counted
    ...
    fun `preview aggregates duplicate ticker across rows`() {
        // Same account + ticker twice (e.g., split lot) → merged into one item
        val file = makeCsv(
