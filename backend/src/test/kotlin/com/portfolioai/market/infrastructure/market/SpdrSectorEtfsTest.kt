...
class SpdrSectorEtfsTest {
  ...
  @Test
  fun `Finnhub-flavoured industries collapse to the right SPDR sector`() {
    // Finnhub's `finnhubIndustry` taxonomy is finer-grained than GICS — multiple sub-industries
    // map to the same SPDR sector. We pin the most common collapses observed in production so a
    // refactor of the synonym table doesn't silently start returning 404 for legitimate symbols.
    assertEquals("XLF", SpdrSectorEtfs.resolve("Banks")?.etfSymbol) // JPM, BAC…
    assertEquals("XLF", SpdrSectorEtfs.resolve("Insurance")?.etfSymbol) // AIG, MET…
    assertEquals("XLV", SpdrSectorEtfs.resolve("Pharmaceuticals")?.etfSymbol) // PFE, MRK…
    assertEquals("XLV", SpdrSectorEtfs.resolve("Biotechnology")?.etfSymbol) // AMGN, GILD…
    assertEquals("XLY", SpdrSectorEtfs.resolve("Retail")?.etfSymbol) // AMZN, NKE…
    assertEquals("XLC", SpdrSectorEtfs.resolve("Media")?.etfSymbol) // DIS, NFLX…
    assertEquals("XLI", SpdrSectorEtfs.resolve("Transportation")?.etfSymbol) // UPS, DAL…
    assertEquals("XLE", SpdrSectorEtfs.resolve("Oil & Gas")?.etfSymbol) // XOM, CVX…
    assertEquals("XLRE", SpdrSectorEtfs.resolve("REIT")?.etfSymbol)
    assertEquals("XLK", SpdrSectorEtfs.resolve("Semiconductors")?.etfSymbol) // NVDA, AVGO…
  }
