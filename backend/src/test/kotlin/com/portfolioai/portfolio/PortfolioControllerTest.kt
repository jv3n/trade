...
class PortfolioControllerTest {
    ...
    private fun assetDto() = AssetDto(
        ...
        quantity = BigDecimal("10"), avgBuyPrice = BigDecimal("150.00"),
    ...
    fun `GET portfolio by id returns 404 when not found`() {
        given(portfolioQueryService.findById(id)).willThrow(NoSuchElementException("Portfolio $id not found"))
    ...
    fun `GET assets returns 404 when portfolio not found`() {
        given(portfolioQueryService.findAssets(id)).willThrow(NoSuchElementException("Portfolio $id not found"))
