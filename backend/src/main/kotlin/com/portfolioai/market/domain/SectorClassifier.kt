 ...
 * implementations live in `infrastructure/market/` : `MockSectorClassifier` (default, hand-curated
 * table) and `FinnhubSectorClassifier` (live REST `/stock/profile2`). Selection via
 * `market.provider`. The "live" path uses Finnhub rather than Twelve Data because Twelve Data's
 * `/profile` is paid-tier only — see `RoutingSectorClassifier` for the routing rationale.
 *
 * Contract :
 * - **Input is trimmed + uppercase**. Callers (currently `SectorClassifierService` via
 *   `MarketController`) normalise once at the boundary ; adapters trust this and don't
 *   re-normalise. Cf. audit 2026-05-06 finding "coutures benchmark v2".
