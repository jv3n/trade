...
/**
 * Boot-time cleanup of orphaned async narrative jobs. A `PENDING` row in `ticker_narrative_job` is
 * the byproduct of "request received, runner kicked, LLM call in-flight" — if the backend hot-
 * reloads (Tilt save) or crashes between "kick" and "complete", the row stays `PENDING` forever and
 * the front polls it indefinitely until the abort timeout fires.
 *
 ...
 */
@Component
class OrphanedJobCleanupListener(private val narrativeJobRepository: TickerNarrativeJobRepository) {
  ...
  fun cleanupOrphanedJobs() {
    ...
    if (flipped > 0) {
      log.info("Orphaned-job cleanup at boot : narrative={}", flipped)
    }
  }
