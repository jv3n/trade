 ...
 *   `UrlTree` pointing at `/dashboard` (USER stumbled onto an ADMIN URL — we don't show a 403
...
describe('adminGuard', () => {
  ...
  it('returns a UrlTree to /dashboard when the user is USER', () => {
  ...
  it('returns a UrlTree to /dashboard when no user is authenticated', () => {
    ...
    // unauthenticated user is by definition not admin, so we send them to /dashboard (which the
