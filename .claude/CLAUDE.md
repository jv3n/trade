...
- `features/` — UI feature folders (one per top-level route, *primary adapters* en vocabulaire hexagonal) :
  ...
  - `settings/` — back-office avec sidenav : `sources/` (activer/désactiver), `test-sources/` (tester un flux), `prompt-preview/` (aperçu du prompt LLM), `configuration/` (config runtime — sub-sidenav interne « Providers de données » / « LLM », signal `activeSection` persistée localStorage `runtime-config-section`)
