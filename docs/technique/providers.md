...
| **Endpoints utilisés** | `/company-news?symbol=...&from=...&to=...` (Phase 2 news), `/stock/recommendation?symbol=...` (Phase 2 analyst — breakdown monthly), `/stock/price-target?symbol=...` (Phase 2 analyst — price target 12 mois, fail-soft à `null` sur 401/403 paid tier), `/stock/earnings?symbol=...` (Phase 2 earnings — historique 4 derniers Q EPS estimate/actual/surprise), `/calendar/earnings?from=...&to=...&symbol=...` (Phase 2 earnings — prochaine annonce sur fenêtre 90 j, fail-soft à `null` sur 401/403 paid tier) |
| **Config** | `news.provider: finnhub` (news) + `analyst.provider: finnhub` (recos analystes) + `earnings.provider: finnhub` (earnings) + `market.finnhub.api-key` (env `FINNHUB_API_KEY`) — clé partagée, trois toggles indépendants |
| **Adapter** | `FinnhubClient` (sélectionné par `news.provider: finnhub`) + `FinnhubAnalystClient` (sélectionné par `analyst.provider: finnhub`) + `FinnhubEarningsClient` (sélectionné par `earnings.provider: finnhub`) |
...
| **Cache** | Caffeine `news-by-symbol` (TTL 15 min, key `{symbol}\|{limit}`) + `analyst-recommendations` (TTL 15 min, key `{symbol}`) + `earnings` (TTL 15 min, key `{symbol}`) — TTL partagé via `market.cache.ttl-minutes` |
