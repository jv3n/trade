...
| **Endpoints utilisés** | `/time_series` (OHLC), `/quote` (nom + 52w + close) |
| **Config** | `market.twelvedata.api-key` (env `TWELVEDATA_API_KEY`) |
| **Adapter** | `TwelveDataClient` (sélectionné par `market.provider: twelvedata`) |
| **Cache** | Caffeine `market-chart`, TTL 15 min, key `twelvedata\|{symbol}\|{range}\|{interval}` |
...
| **Modèle par défaut** | `qwen2.5:3b` (~2 Go, ~5-10 s par narratif sur M1) — pull via le bouton Tilt **`llm:pull-qwen`** |
